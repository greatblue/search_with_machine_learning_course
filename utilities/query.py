...
model = fasttext.load_model("../week3/query_classifier_ph.bin") 
...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc", is_boost=False, category_threshold=0.3):
    ...
    stemmed_tokens = list(map(lambda x: stemmer.stem(x), normalized_query_tokens))
    ...
    query_obj = create_query(user_query, click_prior_query=None, filters=filters, sort=sort, sortDir=sortDir, source=["name", "shortDescription"], categories=category_list, is_boost=is_boost)
...
if __name__ == "__main__":
    ...
    general.add_argument("-b", '--boost', default=False, action='store_true', 
                        help='Determines whether predicted categories whould be used for boosting')
    general.add_argument("-c", '--catthreshold', type=float, default=0.3, 
                        help='Probability threshold for categories')
